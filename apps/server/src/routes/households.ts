...
householdsRouter.post("/", requireWritableAccount, async (req, res) => {
...
// Write-guarded: it persists onto the User row, and a shared read-only login
// switching household would yank the view out from under every other visitor
// using it at the same time.
householdsRouter.post("/active", requireWritableAccount, async (req, res) => {
  const userId = req.user!.id;
