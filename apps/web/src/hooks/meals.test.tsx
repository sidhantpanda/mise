...
describe("useMeals", () => {
  it("goes loading -> success with the fetched list", async () => {
    server.use(http.get("/api/meals", () => HttpResponse.json([{ identifier: "m1" }])));

    const { result } = renderHook(() => useMeals(), { wrapper });
    expect(result.current.isLoading).toBe(true);

    await waitFor(() => expect(result.current.isSuccess).toBe(true));
    expect(result.current.data).toEqual([{ identifier: "m1" }]);
  });

  it("goes loading -> error on a server failure", async () => {
    server.use(http.get("/api/meals", () => HttpResponse.json({ error: "boom" }, { status: 500 })));
